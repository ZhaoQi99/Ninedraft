...
class GameView(tk.Canvas):
    ...
    def show_target(self,
                    ...
                    target_thickness=2,
                    crosshair_radius=4,
                    target_colour='purple',
                    cursor_bg_colour='grey',
                    cursor_fg_colour='white'):
        ...
        self.create_rectangle(
            coords,
            fill='',
            width=target_thickness * 2,
            outline=target_colour,
            tag=('block', 'target'))

        if cursor_bg_colour:
            self.create_line(
                player_position, (cx, cy),
                fill=cursor_bg_colour,
                tag='cursor',
                width=3)

        if cursor_fg_colour:
            self.create_line(
                player_position, (cx, cy), fill=cursor_fg_colour, tag='cursor')
        ...
        if cursor_bg_colour:
            self.create_line(
                horizontal, fill=cursor_bg_colour, tag='cursor', width=3)
            self.create_line(
                vertical, fill=cursor_bg_colour, tag='cursor', width=3)
...
class WorldViewRouter(InstanceRouter):
    ...
    def _draw_block(self, instance, shape, view):
        return [
            view.create_rectangle(
                ...
                shape.bb.right,
                shape.bb.bottom,
                fill=self._block_colours[instance.get_id()],
                tags='block')
        ]
    ...
    def _draw_mayhem_block(self, instance, shape, view):
        return [
            view.create_rectangle(
                ...
                shape.bb.right,
                shape.bb.bottom,
                fill=instance.colours[instance._i],
                tags='block')
        ]
    ...
    def _draw_bird(self, instance, shape, view):
        ...
        return [
            view.create_polygon((centre_x, bb.top), (bb.right, centre_y),
                                (centre_x, bb.bottom), (bb.left, centre_y),
                                fill='#87CEEB',
                                tags=('mob', 'bird'))
        ]
    ...
    def _draw_sheep(self, instance, shape, view):
        ...
        return [
            view.create_polygon((centre_x, bb.top), (bb.right, bb.bottom),
                                (bb.left, bb.bottom),
                                fill='#FFCEEB',
                                tags=('mob', 'sheep'))
        ]
